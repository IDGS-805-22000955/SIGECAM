...
class EtapaProduccion(enum.Enum):
    CORTADO = "CORTADO"
    DOBLILLADO = "DOBLILLADO"
    PESPUNTE = "PESPUNTE"
    MONTADO = "MONTADO"
    ADORNADO = "ADORNADO"
...
class FichaTecnicaInsumos(db.Model):
    __tablename__ = 'ficha_tecnica_insumos'
    id_ficha_insumo = db.Column(db.Integer, primary_key=True, autoincrement=True)
    id_ficha_tecnica = db.Column(db.Integer, db.ForeignKey('ficha_tecnica.id_ficha_tecnica'), nullable=False)
    id_mp = db.Column(db.Integer, db.ForeignKey('materia_prima.id_mp'), nullable=False)

    etapa = db.Column(db.Enum(EtapaProduccion), nullable=False)
    cantidad_requerida = db.Column(db.Numeric(10, 4), nullable=False)
    observacion_material = db.Column(db.String(255), nullable=True)


class FichaTecnicaProcesos(db.Model):
    __tablename__ = 'ficha_tecnica_procesos'
    id_proceso = db.Column(db.Integer, primary_key=True, autoincrement=True)
    id_ficha_tecnica = db.Column(db.Integer, db.ForeignKey('ficha_tecnica.id_ficha_tecnica'), nullable=False)

    etapa = db.Column(db.Enum(EtapaProduccion), nullable=False)
    descripcion_instruccion = db.Column(db.Text, nullable=False)
    maquinaria_sugerida = db.Column(db.String(100), nullable=True)
